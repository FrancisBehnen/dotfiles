...
- **`Pluginfile`** → `claude plugin marketplace add` + `claude plugin install`, restored by `setup.sh` (`install_claude_plugins`). **Plugins provide most of the surface** — `plannotator` (the plannotator-* skills), `frontend-design`, `playwright`, `skill-creator`, `context7` (an MCP), `claude-obsidian` (obsidian suite), `hookify`, `commit-commands`, `feature-dev`, `pr-review-toolkit`, `claude-md-management`, and the `pyright`/`typescript` LSPs. So these are **not** file-tracked — they return with their plugin.
  - ⚠️ **Untested on a clean machine:** `install_claude_plugins` (the `claude plugin marketplace add` / `claude plugin install` step) was written but never run through a fresh `setup.sh`. The commands and idempotency look right, but validate this step on the next real bootstrap before trusting it.
- **`CLIfile`** → `install_cli_tools` in `setup.sh`. Agent-facing **CLI binaries** that skills shell out to but that Brewfile/Skillfile/Pluginfile don't cover: `nlm` (NotebookLM, `uv tool install notebooklm-mcp-cli`), `playwright-cli` + Chromium browsers, `ctx7` (Context7 CLI, backs `find-docs`), `defuddle`, `mcp2cli`. Pipe-delimited `name | verify-cmd | install-cmd`; each line's verify-cmd makes it idempotent. One-time interactive **auth** (`nlm auth`, etc.) is never scripted. Note: `glean`/`fellow`/`slack` CLIs are handled by `setup_pulse_sources` (project-pulse path), **not** the CLIfile — don't duplicate them. Unlike plugins/skills, `install_cli_tools` **has** been run for real (on the primary machine, 2026-07-13) and is confirmed idempotent; the fresh-bootstrap ordering (needs `node`/`bun`/`uv` first) is still unvalidated end-to-end.
